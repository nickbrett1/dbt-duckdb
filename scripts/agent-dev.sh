...
# "dbt-duckdb-dev" (the repo name plus the "-dev"
...
# than adding a second one - a stale "dbt-duckdb-dev" is overwritten by
...
PROJECT_NAME="dbt-duckdb"
AGENT_NAME="dbt-duckdb-dev"
WORKSPACE="/workspaces/dbt-duckdb"
...
if [ -x "/workspaces/dbt-duckdb/scripts/agent-dev.sh" ]; then
  "/workspaces/dbt-duckdb/scripts/agent-dev.sh" start >/dev/null 2>&1 || true
fi
POST_START_HOOK
}
